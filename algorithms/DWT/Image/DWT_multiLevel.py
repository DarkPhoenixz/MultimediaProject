...
def embed_secret_image_dwt_distributed(
    ...
    secret_image_path,
    stego_image_path,
    bits_used=1,            # <--- Use 1 bit
    wavelet='bior2.2',      # <--- Biorthogonal wavelet
    level=3                 # <--- Single-level DWT
):
    """
    Embed a secret image into a cover image by distributing the secret data over
    the detail subbands of a multi-level (here single-level) DWT.
    """
    # Load cover
...
def main(cover_image_path, secret_image_path):
    ...
    axes[1, 0].set_title("Original Secret (resized if needed)")
    axes[1, 0].axis("off")
    axes[1, 1].imshow(secret_extracted, cmap='gray')
    axes[1, 1].set_title("Extracted Secret")
    axes[1, 1].axis("off")
    fig.suptitle(
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uso: python DWT_multiLevel.py original_image secret_image")
        sys.exit(1)
...
#combo migliori: 2/3 2/2
#le bande sono dovute alla modifica dei coeff
#nel caso di gem si fa un forte uso di quelli orizzontali e diagonali
